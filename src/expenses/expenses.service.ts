...
export class ExpensesService {
  ...
  async getReport(
    groupBy: 'route' | 'aircraft' | 'flight' | 'expense_type',
    from?: string,
    to?: string,
  ): Promise<any[]> {
    ...
    if (from) qb.andWhere('DATE(je.entry_date) >= :from', { from })
    if (to)   qb.andWhere('DATE(je.entry_date) <= :to',   { to })
