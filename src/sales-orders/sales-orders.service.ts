...
export interface OrderItem {
  ...
  salesOrderId: number;
  productId: number;
  productName: string; // Fetched from products table via JOIN
  quantity: number;
  unitPrice: number;
  totalPrice: number;
  orderDate: string;
...
export class SalesOrdersService {
  ...
  async getSalesAnalytics(year?: number): Promise<SalesAnalytics> {
    const targetYear = year || new Date().getFullYear();
    const currentDate = new Date();
    const currentMonth = currentDate.getMonth();
    const currentYear = currentDate.getFullYear();
    
    // Use raw query for better performance - get orders for the year with status 1, 2, or 3
    const query = `
      SELECT 
        id,
    ...
    // Calculate total revenue for the current month and year
    let totalRevenue = 0;
    if (targetYear === currentYear) {
      // If requesting current year, show current month revenue
      ...
      // If requesting a different year, show total year revenue
      totalRevenue = orders.reduce((sum, order) => sum + Number(order.totalAmount), 0);
    }

    // Calculate total orders and average order value for the same period
    let totalOrders = 0;
    if (targetYear === currentYear) {
      const currentMonthOrders = orders.filter(order => 
        new Date(order.orderDate).getMonth() === currentMonth
  ...
  async getClientSalesData(clientId: number, year?: number): Promise<ClientSalesData | null> {
    const targetYear = year || new Date().getFullYear();
    
    try {
      // Optimized query that gets client and sales data in one go
      ...
      // Get client info from first result
      ...
      const monthlyData: MonthlySalesData[] = Array.from({ length: 12 }, (_, i) => ({
      ...
      // Process results and group by month
      const monthlyTotals = new Map<number, { orders: number, amount: number, orderList: any[] }>();
  ...
  // Optimized method to get sales data for multiple clients at once
  async getBulkClientSalesData(clientIds: number[], year?: number): Promise<ClientSalesData[]> {
    const targetYear = year || new Date().getFullYear();
    ...
    try {
      ...
      // Simplified query - get all orders for the clients first
      const bulkQuery = `
        SELECT 
          c.id as clientId,
          c.name as clientName,
      ...
      const clientDataMap = new Map<number, ClientSalesData>();
      ...
      results.forEach(row => {
        ...
        if (clientData) {
          ...
          if (row.orderId && row.orderDate) {
            ...
            const monthIndex = orderDate.getMonth(); // 0-based month index
  ...
  async getAllClientSalesData(year?: number): Promise<ClientSalesData[]> {
    const targetYear = year || new Date().getFullYear();
    
    // First, try to get all clients
    let clients: any[] = [];
    try {
      ...
      // If clients table doesn't exist, get clients from sales_orders
      const salesQuery = `
        SELECT DISTINCT 
          client_id as id,
    ...
    // Process each client
    const result: ClientSalesData[] = clients.map(client => {
    ...
    // Sort by total amount (descending), then by name
  ...
  async getClientOrderItems(clientId: number, year: number, month: number): Promise<OrderItem[]> {
    try {
      ...
      // Check for sales orders in the specific year/month
      ...
      // Show actual sales orders for this client
      ...
      // Show all sales orders in the database
      ...
      // If no results and we used p.product_name, try p.name as fallback
      if (result.length === 0) {
  ...
  async getProductPerformance(year?: number): Promise<ProductPerformanceData[]> {
    ...
    try {
      ...
      // Check for sales orders in the target year
      ...
      // Check for sales orders with my_status = 3
      ...
      // Get products that have sales data from sales_order_items where my_status = 3
      const productsQuery = `
        SELECT DISTINCT 
          p.id,
          p.product_name,
      ...
      if (products.length === 0) {
        ...
        // Let's try a broader query to see if there are any products with sales data at all
      ...
      for (const product of products) {
        // Get sales data for this product where my_status = 3
        const query = `
          SELECT 
            p.id as productId,
            p.product_name as productName,
        ...
        // Get overall totals for this product where my_status = 3
        const totalsQuery = `
          SELECT 
            COALESCE(SUM(soi.quantity), 0) as totalQuantitySold,
            COALESCE(SUM(soi.total_price), 0) as totalRevenue,
  ...
  async getProductPerformanceSummary(year?: number): Promise<ProductPerformanceSummary> {
    ...
    try {
      ...
      // Get top performing product where my_status = 3
